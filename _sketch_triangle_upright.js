...
function drawTessellation() {
  ...
  const rows = Math.ceil(height / triangleHeight) + 2; // Number of rows

  for (let row = -rows; row < rows; row++) {
    for (let col = -cols; col < cols; col++) {
      const xOffset = col * triangleBase; // Horizontal offset
      const baseYOffset = row * (triangleHeight / 2) + lineYOffsetAdjustment; // Base vertical offset
      const yOffset = baseYOffset + (row % 2 !== 0 ? triangleHeight / 6 : 0); // Adjust flipped rows down

      // Alternate row alignment
      const isFlipped = row % 2 !== 0;
      const adjustedXOffset = xOffset + (isFlipped ? triangleBase / 2 : 0);

      drawTriangleAt(adjustedXOffset, yOffset, isFlipped);
    }
  }
}
