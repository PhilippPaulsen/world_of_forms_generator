...
function drawTessellation() {
  ...
  for (let row = -rows; row < rows; row++) {
    for (let col = -cols; col < cols; col++) {
      ...
      const yOffset = row * (triangleHeight / 2); // Base vertical offset

      // Alternate row alignment
      const isFlipped = row % 2 !== 0;
      const adjustedXOffset = xOffset + (isFlipped ? triangleBase / 2 : 0);
      const adjustedYOffset = yOffset + (isFlipped ? triangleHeight / 6 : 0); // Fine-tuned adjustment for flipped rows

      drawTriangleAt(adjustedXOffset, adjustedYOffset, isFlipped);
    }
  }
}
