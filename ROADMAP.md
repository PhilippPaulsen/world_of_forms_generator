...
- Raise the node-count UI limit (`#node-count-input`, currently hardcoded 1–5) — not risk-free above ~6–8: `updatePatternNameStatus()`'s orbit-table computation (`core/orbits.js`) reruns uncached on every connection click, not just on a node-count change, and grows fast with node count, especially for hex (~3n² nodes) — measured 21ms at hex order 5 (today's max) up to 6.5s at order 20. Raise to a moderate value (6–8), backed by this measurement; `isWellFormedCatalogPattern()`'s own independent order-range check (`sketch.js:82`) must be updated in the same step. A materially higher limit needs an orbit-table cache first (analogous to `1.8` Phase (iv)'s `getGroupElementsCached()`), not bundled into this pass.
- Slider controls for the curve parameters — only `roughness` (`1.5-A`) is a genuine widget swap: already continuous end-to-end, already has a number input. `fold` must stay a positive integer per Ostwald's "1-/2-/3-fold" counting (a stepped range, not a continuous slider), and neither `fold` nor `strength` has any live control today — both are hardcoded once at curve-mode activation and never re-read, so adding controls for them is new UI, not a swap. `seed` is dropped from this group: `1.5-B`'s reroll-button design was a deliberate choice ("a seed has no meaningful order to tune"), not an oversight.
