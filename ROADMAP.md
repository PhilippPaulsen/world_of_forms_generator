...
### 1.2 Alternative net construction — ⛔ not started
...
- *Status: `drawCurvedBezier()` with a single `curveAmount` parameter already implements the simplest (1-fold) case, including SVG export via quadratic Bézier paths. Extending to the remaining 17 cases and compound lines is the open work.*
