...
- **Friction demonstrated in real testing** (Phase 2's deferred items are no longer only theoretical): editing a connection after assigning a palette leaves some trails on the stale series and others silently back on default with no explanation (edit + undo test: 7 trails → 5, of which 2 unexplained defaults and 3 stale-series colors; undo restores everything, confirming orphan-retention was the right call). Split/merge handling is therefore the clear next candidate if this feature continues; two options surfaced, not yet decided: (a) real inheritance via face-overlap heuristics, or (b) automatic palette redistribution on pattern edit.
...
- **Group D's original four-phase plan (engine → key + store → UI → export) is now fully shipped end to end.** What remains is the open decisions above (chiefly split/merge handling) and items 3–4 of the list (per-layer coloring beyond the per-sheet stores already built, and color as an animatable timeline parameter).
